...
async function fetchImageFromS3AsObjectURL(bucketName: string, objectKey: string): Promise<string | undefined> {
  try {
...
function extractS3Details(s3Uri: string) {
...
fetchImageFromS3AsObjectURL(bucketName, objectKey).then((objectUrl) => {
  if (objectUrl) {
    ...
    const myImgElement = document.getElementById('my-img') as HTMLImageElement; 
